...
import { updateEvent, getEvent, clearEvent } from "../store/action/eventAction";
...
const UpdateEvent = ({ updateEvent, eventDetail, }) => {
  ...
  const { register, handleSubmit, formState: { errors }, control } = useForm({
    ...
    defaultValues: {
      ...
      start: new Date(eventDetail.start),
      end: eventDetail.end ? new Date(eventDetail.end) : "",
      description: eventDetail.description ? eventDetail.description : "No description was provided"
    }
  });
  ...
  const onSubmit = async (values) => {
    ...
    await updateEvent(eventDetail._id, payload)
    navigate('/schedule');
  }
    ...
    <figure className="py-10 flex bg-gray-100">
      <div className="w-full max-w-md m-auto bg-white rounded-lg border border-primaryBorder py-8 px-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className=" align-content-center m-5">
          ...
          <button type="submit" className="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update</button>
        </form>
      </div>
    </figure>
  )
...
export default connect(mapStateToProps, { updateEvent, getEvent, clearEvent })(UpdateEvent);